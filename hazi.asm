...
;< ki- és bemenetek inicializálása stb > 
	...
	ldi pwm, 0x00 ; kis kitöltési tényezőjű jel 
	out OCR0, pwm  
	...
	ldi counter, 0x80 	; 
DELAY:					; Várakozás fényérzékelőre	
	dec counter			;
	brne DELAY 			;
	
	call LUX_M		;
	cp lux, sotet	; Ha kellően sötét van akkor esti üzemmód
	brlo ESTE		;
;*************************************************************** 
...
ESTE:
	...
	ldi temp, 0b01001100 ; Fast PWM mód, 64-es előosztó 
	out TCCR0, temp
...
LOOP:
	...
	call LUX_M		;	
	cp lux, sotet	; Hogy ne kelljen várni újabb fényerő vizsgálatra
...
T2CM_IT: 				; Ha végzett a számolással Timer/Counter2 
	...
	cpi pwm, 0			; Ha pwm elérte 0x00-át akkor nőjjön
	breq NO
